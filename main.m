...
% X = rand_sampling(database, nsmp);
% 
% [B S stat] = Lsparse_coding(X, nBases, beta, sparsity_func, epsilon, num_iters, batch_size, Binit);
% [h, array] = display_network(B)
...
% [~, ~, class_name] = li2nsvm_multiclass_lbfgs(last_fea', sc_label, lambda);
last_fea = last_fea';
for ii = 1:nRounds
    fprintf('Round: %d...\n', ii);
    tr_idx = [];
